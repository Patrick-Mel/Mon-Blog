...
import { Bold, Italic, Code, Heading1, Heading2, List, Link as LinkIcon, Image as ImageIcon, Save, ArrowLeft, Sparkles, CheckCircle2 } from 'lucide-react';
...
export function ArticleEditor({ initialArticle, categories }: ArticleEditorProps) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose-custom min-h-[300px] focus:outline-none p-4 text-slate-900 dark:text-slate-200',
      },
    },
  });
  ...
  return (
    <div className="space-y-8 max-w-5xl mx-auto pb-16">
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-4">
        <h3 className="text-sm font-bold text-slate-900 dark:text-white border-b border-slate-200 dark:border-slate-800 pb-2">Informations Générales</h3>

        <div>
          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Titre de l'article *</label>
          <input
            type="text"
            placeholder="Ex: Guide Ultime Next.js 15 & Supabase RLS"
            value={titre}
            onChange={e => handleTitreChange(e.target.value)}
            className="w-full px-4 py-3 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-base font-bold text-slate-900 dark:text-white focus:border-amber-500 focus:outline-none"
          />
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              className="w-full px-3.5 py-2.5 rounded-xl bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-800 text-xs font-mono text-slate-800 dark:text-slate-300"
            />
          </div>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Extrait (Chapeau de l'article)</label>
          <textarea
            rows={2}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Statut de Publication</label>
            <select
              value={statut}
      ...
      <div className="glass rounded-2xl p-6 border border-slate-200 dark:border-slate-800 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 dark:border-slate-800 pb-2">
          <h3 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Sparkles className="w-4 h-4 text-amber-500" /> Monétisation & Sponsoring Article
          </h3>
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-xs font-bold text-amber-600 dark:text-amber-400">Article Sponsorisé</span>
          </label>
        </div>
        ...
        {sponsorise && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in duration-200">
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Nom du Sponsor / Partenaire</label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1">Lien Cible du Sponsor (rel="sponsored")</label>
              <input
                type="text"
      ...
      <div className="glass rounded-2xl border border-slate-200 dark:border-slate-800 overflow-hidden space-y-0 shadow-xl">
        {/* Toolbar */}
        {editor && (
          <div className="p-3 bg-slate-100 dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex flex-wrap items-center gap-1 text-slate-700 dark:text-slate-300">
            <button
              onClick={() => editor.chain().focus().toggleBold().run()}
              className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 ${editor.isActive('bold') ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : ''}`}
              title="Gras"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 ${editor.isActive('italic') ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : ''}`}
              title="Italique"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 ${editor.isActive('heading', { level: 2 }) ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : ''}`}
              title="Titre H2"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 ${editor.isActive('heading', { level: 3 }) ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : ''}`}
              title="Titre H3"
            >
            ...
            <button
              ...
              className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 ${editor.isActive('codeBlock') ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : ''}`}
            ...
            <button
              ...
              className={`p-2 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-800 ${editor.isActive('bulletList') ? 'bg-amber-500/20 text-amber-600 dark:text-amber-400' : ''}`}
              title="Liste à puces"
            >
