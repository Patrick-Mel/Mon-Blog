          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-200 dark:divide-slate-800">
              {articles.map(article => (
                <tr key={article.id} className="hover:bg-slate-100 dark:hover:bg-slate-800/40 transition-colors">
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    {article.statut === 'publie' && (
                      <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-semibold text-[10px]">
                        Publié
                      </span>
                    )}
                    {article.statut === 'brouillon' && (
                      <span className="px-2.5 py-0.5 rounded-full bg-amber-500/20 text-amber-600 dark:text-amber-400 font-semibold text-[10px]">
                        Brouillon
                      </span>
                    )}
                    {article.statut === 'archive' && (
                      <span className="px-2.5 py-0.5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-700 dark:text-slate-300 font-semibold text-[10px]">
                        Archivé
                      </span>
                    )}
                  </td>
                  <td className="p-4 font-mono text-slate-700 dark:text-slate-300">{article.vues_count}</td>
                  <td className="p-4">
                    {article.sponsorise ? (
                      <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-600 dark:text-amber-300 text-[10px] font-bold flex items-center gap-1 w-fit">
                  ...
                  <td className="p-4 text-right">
                    <div className="flex items-center justify-end gap-2">
                      ...
                      <Link
                        ...
                        className="p-2 rounded-lg bg-amber-500/20 text-amber-600 dark:text-amber-400 hover:bg-amber-500 hover:text-slate-950 transition-colors"
                        title="Modifier"
                      >
