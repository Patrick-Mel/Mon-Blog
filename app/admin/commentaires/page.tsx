    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      <div>
        <h1 className="text-2xl font-black text-white flex items-center gap-2">
          <MessageSquare className="w-6 h-6 text-amber-400" /> Modération des Commentaires
        </h1>
        <p className="text-xs text-slate-400">Approuver, rejeter ou supprimer les contributions des lecteurs</p>
      </div>
      ...
      <div className="space-y-4">
        {filteredComments.length === 0 ? (
          <div className="glass p-8 rounded-2xl text-center text-xs text-slate-400">
            Aucun commentaire à afficher.
          </div>
        ) : (
          filteredComments.map(comment => (
            <div key={comment.id} className="glass p-6 rounded-2xl border border-slate-800 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <div className="text-xs font-bold text-white">{comment.auteur_nom}</div>
                    <div className="text-[10px] text-slate-400">{comment.auteur_email}</div>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {comment.statut === 'en_attente' && (
                    <span className="px-2.5 py-0.5 rounded bg-amber-500/20 text-amber-400 font-bold text-[10px]">
                      En attente
                    </span>
                  )}
              ...
              <p className="text-xs text-slate-300 bg-slate-950 p-3 rounded-xl border border-slate-900">
                "{comment.contenu}"
              </p>
              ...
              <div className="flex items-center justify-between pt-2 border-t border-slate-800">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600 hover:text-white text-xs font-bold transition-all flex items-center gap-1"
                  >
                    <Check className="w-3.5 h-3.5" /> Approuver
                  </button>
                  <button
                    onClick={() => handleAction(comment.id, 'rejete')}
                    className="px-3 py-1.5 rounded-lg bg-red-600/20 text-red-400 hover:bg-red-600 hover:text-white text-xs font-bold transition-all flex items-center gap-1"
                  >
